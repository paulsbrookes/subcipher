...
def rm_duplicate_ciphertexts3(values, number_retained):
...
def rm_duplicate_ciphertexts(ciphertexts, retain=0):
    if retain == 0:
        retain = len(ciphertexts)
    added = []
    number_added = 0
    filtered_ciphertexts = []
    for i, x in enumerate(ciphertexts):
        ...
        if map not in added:
            ...
            if number_added == retain:
                return filtered_ciphertexts
    return filtered_ciphertexts

def rm_duplicate_records(objects_list, retain=1):
    added = []
    number_added = 0
    filtered_objects = []
    for i, object in enumerate(objects_list):
        map = object.map_record.tolist()
        if map not in added:
            number_added += 1
            added.append(map)
            filtered_objects.append(objects_list[i])
            if number_added == retain:
...
def transposition_matrix(map):
    matrix = np.zeros([len(map),len(map)])
